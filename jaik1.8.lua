...
local espToggle = CreateToggle(VisualsTab, "2D Box ESP", function(enabled)
    ESP.Enabled = enabled
    if not enabled then
        ClearESP()
    end
...
local function Create2DBox(player)
    ...
    healthText.Position = UDim2.new(0, 0, 1, 4)
...
local function Update2DBox(player)
    ...
    if humanoid and head then
        ...
        if onScreen then
            ...
            local width = math.clamp(scale * 0.8, 50, 150)
            local height = math.clamp(scale * 1.2, 100, 200)
            ...
            box.ESP_HealthBar.Size = UDim2.new(healthPercent, 0, 0, 4)
