...
local function TrackPlayer(player)
    ...
    connections.characterAdded = player.CharacterAdded:Connect(function(character)
        ...
        if ESP.Enabled then
            if ESP.Box2D then Create2DBox(player) end
            if ESP.Box3D then Create3DBox(player) end
            if ESP.Chams then CreateChams(player) end
        end
    end)
...
local function Update3DBox(player)
    ...
    if box and player.Character and player.Character:FindFirstChild("Humanoid") and player.Character.Humanoid.Health > 0 then
        if ESP.Colors.Team and player.Team then
...
local function UpdateChams(player)
    ...
    if highlight and player.Character and player.Character:FindFirstChild("Humanoid") and player.Character.Humanoid.Health > 0 then
        if ESP.Colors.Team and player.Team then
...
print("ExpensiveMods fully loaded! All functions are working properly.")
